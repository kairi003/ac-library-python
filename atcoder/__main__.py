...
def safe_cast(type_: type[T], value: Any) -> T:
    """Assert that `value` is of type `type_`."""
    if not isinstance(value, type_):
        raise TypeError(f"Expected {type_}, but {value} is instance of {type(value)}")
    return value


def iter_child_nodes(node: ast.AST, import_info: ImportInfo | None = None) -> list[ImportInfo]:
    result = []
        ...
        for name in node.names:
            if re.match(r"^atcoder\.?", name.name):
                if hasattr(node, "end_lineno"):
                    end_lineno = safe_cast(int, node.end_lineno)
                else:
                    end_lineno = node.lineno
                import_info = ImportInfo(node.lineno, end_lineno)
    elif isinstance(node, ast.ImportFrom):
        if re.match(r"^atcoder\.?", safe_cast(str, node.module)):
            if hasattr(node, "end_lineno"):
                end_lineno = safe_cast(int, node.end_lineno)
            else:
                end_lineno = node.lineno
...
class ModuleImporter:
    ...
    def import_module(self, import_from: str | None, name: str, asname: str | None = None) -> str:
        result = ""
        ...
        if module_name not in self.imported_modules:
            ...
            for import_info in imports:
                result += self.import_module(
                    import_info.import_from,
                    safe_cast(str, import_info.name),
                    import_info.asname,
                )
            ...
            for import_info in imports:
                if import_info.import_from is None:
                    modules = safe_cast(str, import_info.name).split(".")
                    for i in range(len(modules)):
                        import_name = ".".join(modules[: i + 1])
                        if import_name in imported:
                            continue
                        imported.append(import_name)
                        result += f"{module_name}.__dict__['{import_name}'] = {import_name}\n"
                else:
                    result += (
                        f"{module_name}.__dict__['{import_info.name}'] = {import_info.import_from}.{import_info.name}\n"
                    )
